/**
 * Apply gluten allergen rules for brunch items from official allergy sheet
 */
...
// Rules from official gluten allergy sheet - BRUNCH section only
const BRUNCH_GLUTEN_RULES: GlutenRule[] = [
  {
...
async function applyGlutenRules() {
  ...
  console.log('🔄 Applying gluten allergen rules for brunch...\n');
  ...
  console.log('Processing items from gluten sheet:\n');

  // Apply rules for items ON the sheet
  for (const rule of BRUNCH_GLUTEN_RULES) {
  ...
  console.log('\n' + '─'.repeat(60));
  console.log('Processing items NOT on gluten sheet (marking as NOT SAFE):\n');

  // Mark items NOT on sheet as NOT SAFE
  const itemsOnSheet = BRUNCH_GLUTEN_RULES.map(r => r.itemName);
  ...
  for (const item of itemsNotOnSheet) {
    ...
    console.log(`🚫 ${item.name}: NOT IN SHEET (cannot be made gluten-safe)`);
    notSafeCount++;
  }

  console.log('\n' + '='.repeat(60));
  console.log('✅ Gluten rules applied!');
  console.log(`   Items with gluten rules: ${appliedCount}`);
  console.log(`   Items NOT in gluten sheet: ${notSafeCount}`);
